...
@Component({
  standalone: true,
  template: `<th [cpsTColSortable]="field"><span>Name</span></th>`,
  imports: [CpsTableColumnSortableDirective]
})
class TestHostComponent {
  @ViewChild(CpsTableColumnSortableDirective)
  directive!: CpsTableColumnSortableDirective;

  field = 'name';
}

describe('CpsTableColumnSortableDirective', () => {
  let fixture: ComponentFixture<TestHostComponent>;
  let host: TestHostComponent;
  let directive: CpsTableColumnSortableDirective;
  ...
  function buildMockTable() {
    ...
    return {
      ...
      _multiSortMeta: undefined as unknown,
      showInitialSortBadge: false,
      groupRowsBy: undefined as unknown
    };
  }
  ...
  describe('field input', () => {
    it('should set field via cpsTColSortable alias', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('should subscribe to sortSource$', () => {
      expect((directive as any)._sortSub).toBeTruthy();
    });
  });
  ...
  describe('_updateAriaSort (via sortSource$ or init)', () => {
    ...
    it('should set aria-sort to "ascending" in single mode when sortOrder is 1', () => {
      mockTable.sortMode = 'single';
      mockTable.sortOrder = 1;
      mockTable.isSorted.mockReturnValue(true);
    ...
    it('should set aria-sort to "none" in single mode when field is not sorted', () => {
      mockTable.sortMode = 'single';
      mockTable.isSorted.mockReturnValue(false);
      sortSourceSubject.next(null);
    ...
    it('should set aria-sort to "ascending" in multiple mode when meta order is 1', () => {
      ...
      mockTable.getSortMeta.mockReturnValue({ field: 'name', order: 1 });
      sortSourceSubject.next(null);
      expect(th.getAttribute('aria-sort')).toBe('ascending');
    });
    ...
    it('should set aria-sort to "none" in multiple mode when getSortMeta returns null', () => {
      mockTable.sortMode = 'multiple';
      mockTable.getSortMeta.mockReturnValue(null);
      sortSourceSubject.next(null);
      expect(th.getAttribute('aria-sort')).toBe('none');
    });
  ...
  describe('onClick', () => {
    ...
    it('should call dataTable.sort with the current field', () => {
      directive.onClick(new MouseEvent('click'));
      expect(mockTable.sort).toHaveBeenCalledWith({ field: 'name' });
    });
  ...
  describe('ngOnDestroy', () => {
    it('should unsubscribe from sortSource$', () => {
      const sub = (directive as any)._sortSub;
