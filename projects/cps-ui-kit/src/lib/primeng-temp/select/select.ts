 ...
 * Original file: packages/primeng/src/select/select.ts
...
import { deepEquals, equals, findLastIndex, findSingle, focus, getFirstFocusableElement, getFocusableElements, getLastFocusableElement, isEmpty, isNotEmpty, isPrintableCharacter, resolveFieldData, scrollInView, uuid } from '../../primeuix-temp/utils/src/index';
import { FilterService, OverlayOptions, PrimeTemplate, ScrollerOptions, SharedModule, TranslationKeys } from '../api/public_api';
import { AutoFocus } from '../autofocus/public_api';
import { BaseComponent, PARENT_INSTANCE } from '../basecomponent/public_api';
import { BaseInput } from '../baseinput/public_api';
import { Bind, BindModule } from '../bind/public_api';
import { unblockBodyScroll } from '../dom/public_api';
import { IconField } from '../iconfield/public_api';
import { BlankIcon, CheckIcon, ChevronDownIcon, SearchIcon, TimesIcon } from '../icons/public_api';
import { InputIcon } from '../inputicon/public_api';
import { InputText } from '../inputtext/public_api';
import { Overlay } from '../overlay/public_api';
    ...
    template: `
        ...
        >
            <ng-container *ngIf="checkmark">
                <svg data-p-icon="check" *ngIf="selected" [class]="cx('optionCheckIcon')" [pBind]="$safeNavigationMigration($pcSelect?.ptm('optionCheckIcon'))" />
                <svg data-p-icon="blank" *ngIf="!selected" [class]="cx('optionBlankIcon')" [pBind]="$safeNavigationMigration($pcSelect?.ptm('optionBlankIcon'))" />
            </ng-container>
            <span *ngIf="!template" [pBind]="$safeNavigationMigration($pcSelect?.ptm('optionLabel'))">{{ label ?? 'empty' }}</span>
...
@Component({
    ...
    template: `
        ...
        <p-overlay
            ...
            (visibleChange)="overlayVisible = $event"
