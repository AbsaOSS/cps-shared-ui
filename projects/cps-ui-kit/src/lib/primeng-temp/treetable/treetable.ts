 ...
 * Original file: packages/primeng/src/treetable/treetable.ts
...
import {
    ...
    reorderArray,
    resolveFieldData
} from '../../primeuix-temp/utils/src/index';
import { BlockableUI, FilterMetadata, FilterService, PrimeTemplate, ScrollerOptions, SharedModule, SortMeta, TreeNode, TreeTableNode } from '../api/public_api';
...
import { ArrowDownIcon, ArrowUpIcon, CheckIcon, ChevronDownIcon, ChevronRightIcon, SortAltIcon, SortAmountDownIcon, SortAmountUpAltIcon, SpinnerIcon } from '../icons/public_api';
import { PaginatorModule } from '../paginator/public_api';
import { Ripple } from '../ripple/public_api';
import { Scroller } from '../scroller/public_api';
...
} from '../types/treetable/public_api';
    ...
    template: `
        ...
        <p-paginator
            ...
            *ngIf="paginator && (paginatorPosition === 'top' || paginatorPosition == 'both')"
            [templateLeft]="$any(paginatorLeftTemplate ?? _paginatorLeftTemplate)"
            [templateRight]="$any(paginatorRightTemplate ?? _paginatorRightTemplate)"
            [appendTo]="paginatorDropdownAppendTo"
            [currentPageReportTemplate]="currentPageReportTemplate"
            [showFirstLastIcon]="showFirstLastIcon"
            [dropdownItemTemplate]="$any(paginatorDropdownItemTemplate ?? _paginatorDropdownItemTemplate)"
        ...
        <div [pBind]="ptm('scrollableWrapper')" [class]="cx('scrollableWrapper')" *ngIf="scrollable">
            <div
                ...
                *ngIf="frozenColumns || frozenBodyTemplate || _frozenBodyTemplate"
                ...
                [ttScrollableView]="$any(frozenColumns)"
                [unstyled]="unstyled()"
                [frozen]="true"
...
@Component({
    ...
    standalone: false,
    template: `
        <ng-template ngFor let-serializedNode let-rowIndex="index" [ngForOf]="serializedNodes || tt.serializedValue" [ngForTrackBy]="$any(tt.rowTrackBy)">
...
@Component({
    ...
    template: `
        ...
        >
            ...
            <ng-container *ngIf="tt.loaderTemplate || tt._loaderTemplate">
                <ng-template #loader let-scrollerOptions="options">
                    <ng-container *ngTemplateOutlet="tt.loaderTemplate || tt._loaderTemplate; context: $any({ options: scrollerOptions })"></ng-container>
                </ng-template>
            </ng-container>
        </p-scroller>
        <ng-container *ngIf="!tt.virtualScroll">
            ...
            >
                <ng-container *ngTemplateOutlet="buildInItems; context: { $implicit: $any(this).serializedValue, options: {} }"></ng-container>
            </div>
        </ng-container>

        <ng-template #buildInItems let-items let-scrollerOptions="options">
            <table role="treegrid" #scrollTable [pBind]="ptm('table')" [class]="$any(tt.tableStyleClass)" [ngClass]="scrollerOptions.contentStyleClass" [ngStyle]="tt.tableStyle" [style]="scrollerOptions.contentStyle">
                <ng-container
                    *ngTemplateOutlet="frozen ? tt.frozenColGroupTemplate || tt._frozenColGroupTemplate || tt.colGroupTemplate || tt._colGroupTemplate : tt.colGroupTemplate || tt._colGroupTemplate; context: { $implicit: columns }"
                ></ng-container>
                <tbody
...
@Component({
    ...
    standalone: false,
    template: `
        <p-checkbox [ngModel]="checked" [pt]="ptm('pcRowCheckbox')" (onChange)="onClick($any($event))" [binary]="true" [disabled]="$any(disabled)" [indeterminate]="partialChecked" [styleClass]="cx('pcNodeCheckbox')" [tabIndex]="-1" [unstyled]="unstyled()">
...
@Component({
    ...
    standalone: false,
    template: `
        <p-checkbox [ngModel]="checked" [pt]="ptm('pcHeaderCheckbox')" (onChange)="onClick($any($event))" [binary]="true" [disabled]="!tt.value || tt.value.length === 0" [unstyled]="unstyled()">
