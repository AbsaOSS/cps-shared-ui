 ...
 * Original file: packages/primeng/src/datepicker/datepicker.ts
...
import { blockBodyScroll, ConnectedOverlayScrollHandler, unblockBodyScroll } from '../dom/public_api';
import { CalendarIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, TimesIcon } from '../icons/public_api';
...
import {
    ...
    Month,
    NavigationState
} from '../types/datepicker/public_api';
    ...
    template: `
        <ng-template [ngIf]="!inline">
            ...
            <ng-container *ngIf="iconDisplay === 'input' && showIcon">
                <span [class]="cx('inputIconContainer')" [pBind]="ptm('inputIconContainer')" [attr.data-p]="$any(this).inputIconDataP">
        ...
        <p-motion [visible]="$any(inline || overlayVisible)" name="p-anchored-overlay" [appear]="!inline" [options]="computedMotionOptions()" (onBeforeEnter)="onOverlayBeforeEnter($any($event))" (onAfterLeave)="onOverlayAfterLeave($any($event))">
            <div
                #contentWrapper
                [attr.id]="panelId"
                [ngStyle]="panelStyle"
            ...
            >
                                    ...
                                    <tr *ngFor="let week of month.dates; let j = index" [pBind]="ptm('tableBodyRow')">
                                        <td *ngIf="showWeek" [class]="cx('weekNumber')" [pBind]="ptm('weekNumber')">
                                            <span [class]="cx('weekLabelContainer')" [pBind]="ptm('weekLabelContainer')">
                                                {{ $any(month.weekNumbers)[j] }}
                                            </span>
                                        </td>
                                        <td *ngFor="let date of $any(week)" [attr.aria-label]="date.day" [class]="cx('dayCell', { date })" [pBind]="ptm('dayCell')">
                ...
                <div [class]="cx('timePicker')" *ngIf="(showTime || timeOnly) && currentView === 'date'" [pBind]="ptm('timePicker')">
                    <div [class]="cx('hourPicker')" [pBind]="ptm('hourPicker')">
                        ...
                        <span [pBind]="ptm('hour')"><ng-container *ngIf="$any(currentHour) < 10">0</ng-container>{{ currentHour }}</span>
                        <p-button
                            rounded
                            variant="text"
                            severity="secondary"
                    ...
                    <div [class]="cx('minutePicker')" [pBind]="ptm('minutePicker')">
                        ...
                        <span [pBind]="ptm('minute')"><ng-container *ngIf="$any(currentMinute) < 10">0</ng-container>{{ currentMinute }}</span>
                        <p-button
                            rounded
                            variant="text"
                            severity="secondary"
                    ...
                    <div [class]="cx('secondPicker')" *ngIf="showSeconds" [pBind]="ptm('secondPicker')">
                        ...
                        <span [pBind]="ptm('second')"><ng-container *ngIf="$any(currentSecond) < 10">0</ng-container>{{ currentSecond }}</span>
                        <p-button
                            rounded
                            variant="text"
                            severity="secondary"
