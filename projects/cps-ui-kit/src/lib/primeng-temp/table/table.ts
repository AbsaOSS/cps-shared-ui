 ...
 * Original file: packages/primeng/src/table/table.ts
...
import { Button, ButtonModule } from '../button/public_api';
import { CheckboxChangeEvent, CheckboxModule } from '../checkbox/public_api';
import { DatePickerModule } from '../datepicker/public_api';
import { ConnectedOverlayScrollHandler, DomHandler } from '../dom/public_api';
import { ArrowDownIcon } from '../icons/arrowdown/public_api';
...
import { SortAmountUpAltIcon } from '../icons/sortamountupalt/public_api';
import { SpinnerIcon } from '../icons/spinner/public_api';
import { TrashIcon } from '../icons/trash/public_api';
import { InputNumberModule } from '../inputnumber/public_api';
import { InputTextModule } from '../inputtext/public_api';
import { MotionModule } from '../motion/public_api';
import { PaginatorModule } from '../paginator/public_api';
import { RadioButton, RadioButtonClickEvent, RadioButtonModule } from '../radiobutton/public_api';
import { Scroller, ScrollerModule } from '../scroller/public_api';
import { SelectModule } from '../select/public_api';
import { SelectButtonModule } from '../selectbutton/public_api';
import { Nullable, VoidListener } from '../ts-helpers/public_api';
import {
        ...
        <div #wrapper [class]="cx('tableContainer')" [ngStyle]="sx('tableContainer')" [pBind]="ptm('tableContainer')" [attr.data-p]="dataP">
            <p-scroller
                ...
                *ngIf="virtualScroll"
                ...
                [itemSize]="$any(virtualScrollItemSize)"
                [step]="$any(rows)"
                ...
                [loaderDisabled]="true"
                [showSpacer]="false"
                [showLoader]="$any(loadingBodyTemplate || _loadingBodyTemplate)"
...
@Component({
    ...
    template: `
        <ng-container *ngIf="!dataTable.expandedRowTemplate && !dataTable._expandedRowTemplate">
            <ng-template ngFor let-rowData let-rowIndex="index" [ngForOf]="value" [ngForTrackBy]="$any(dataTable.rowTrackBy)">
                <ng-container
...
export class SortableColumn extends BaseComponent {
    ...
    @HostListener('keydown.enter', ['$any($event)'])
...
export class EditableColumn extends BaseComponent {
    ...
    @HostListener('keydown.tab', ['$any($event)'])
    ...
    @HostListener('keydown.escape', ['$any($event)'])
    ...
    @HostListener('keydown.tab', ['$any($event)'])
    @HostListener('keydown.shift.tab', ['$any($event)'])
    @HostListener('keydown.meta.tab', ['$any($event)'])
    ...
    @HostListener('keydown.arrowdown', ['$any($event)'])
    ...
    @HostListener('keydown.arrowup', ['$any($event)'])
    ...
    @HostListener('keydown.arrowleft', ['$any($event)'])
    ...
    @HostListener('keydown.arrowright', ['$any($event)'])
...
@Component({
    ...
    standalone: false,
    template: `<p-radioButton #rb [(ngModel)]="checked" [disabled]="$any(disabled())" [inputId]="$any(inputId())" [name]="$any(name())" [ariaLabel]="ariaLabel" [binary]="true" [value]="value" (onClick)="onClick($event)" [unstyled]="unstyled()" /> `,
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None
})
...
@Component({
    ...
    standalone: false,
    template: `
        <p-checkbox [(ngModel)]="checked" [binary]="true" (onChange)="onClick($event)" [required]="$any(required())" [disabled]="$any(disabled())" [inputId]="$any(inputId())" [name]="$any(name())" [ariaLabel]="ariaLabel" [unstyled]="unstyled()">
...
@Component({
    ...
    standalone: false,
    template: `
        <p-checkbox [pt]="ptm('pcCheckbox')" [(ngModel)]="checked" (onChange)="onClick($event)" [binary]="true" [disabled]="isDisabled()" [inputId]="$any(inputId())" [name]="$any(name())" [ariaLabel]="ariaLabel" [unstyled]="unstyled()">
        ...
        <div [class]="cx('filter')">
            <p-columnFilterFormElement
                *ngIf="display === 'row'"
                ...
                [field]="field"
                [ariaLabel]="ariaLabel"
                [filterConstraint]="$any(dataTable.filters[$any(field)])"
            ...
            @if (renderOverlay()) {
                <div
                    [pMotion]="$any(showMenu && overlayVisible)"
                    [pMotionAppear]="true"
                    pMotionName="p-anchored-overlay"
                    (pMotionOnBeforeEnter)="onOverlayBeforeEnter($any($event))"
                    (pMotionOnAfterLeave)="onOverlayAnimationAfterLeave($any($event))"
                ...
                >
                    ...
                    <ul *ngIf="display === 'row'; else menu" [class]="cx('filterConstraintList')" [pBind]="ptm('filterConstraintList')">
                        ...
                        <li [class]="cx('filterConstraintSeparator')" [pBind]="ptm('filterConstraintSeparator', { context: { index: $any(this).i } })"></li>
                    ...
                    <ng-template #menu>
                                ...
                                <div>
                                    <p-button
                                        *ngIf="showRemoveIcon"
                                        ...
                                        [buttonProps]="$any($safeNavigationMigration(filterButtonProps.popover?.removeRule))"
                                        [unstyled]="unstyled()"
                                    >
                                        <ng-template #icon>
                        ...
                        @if (isShowAddConstraint) {
                            <p-button
                                ...
                                [buttonProps]="$any($safeNavigationMigration(filterButtonProps.popover?.addRule))"
                                [unstyled]="unstyled()"
                            >
                                <ng-template #icon>
                        ...
                        <div [class]="cx('filterButtonbar')" [pBind]="ptm('filterButtonBar')">
                            <p-button
                                ...
                                *ngIf="showClearButton"
                                ...
                                [buttonProps]="$any($safeNavigationMigration(filterButtonProps.popover?.clear))"
                            ...
                            <p-button
                                *ngIf="showApplyButton"
                                ...
                                [buttonProps]="$any($safeNavigationMigration(filterButtonProps.popover?.apply))"
        ...
        <ng-container *ngIf="filterTemplate; else builtInElement">
            <ng-container
                *ngTemplateOutlet="
                    filterTemplate;
                    context: {
                        $implicit: $any(filterConstraint).value,
