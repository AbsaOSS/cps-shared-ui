 ...
 * Original file: packages/primeng/src/overlay/overlay.ts
...
import { absolutePosition, addClass, appendChild, focus, getOuterWidth, getTargetElement, isTouchDevice, relativePosition, removeClass } from '../../primeuix-temp/utils/src/index';
import { OverlayModeType, OverlayOnBeforeHideEvent, OverlayOnBeforeShowEvent, OverlayOnHideEvent, OverlayOnShowEvent, OverlayOptions, OverlayService, PrimeTemplate, ResponsiveOverlayOptions, SharedModule } from '../api/public_api';
...
import { OverlayContentTemplateContext } from '../types/overlay/public_api';
        ...
        @if (inline()) {
            ...
            <div *ngIf="modalVisible" #overlay [class]="cn(cx('root'), styleClass)" [style]="sx('root')" [pBind]="ptm('root')" (click)="onOverlayClick()">
                <p-motion
                    ...
                    (onBeforeEnter)="onOverlayBeforeEnter($any($event))"
                    (onEnter)="onOverlayEnter($any($event))"
                    (onAfterEnter)="onOverlayAfterEnter($any($event))"
                    (onBeforeLeave)="onOverlayBeforeLeave($any($event))"
                    (onLeave)="onOverlayLeave($any($event))"
                    (onAfterLeave)="onOverlayAfterLeave($any($event))"
                >
